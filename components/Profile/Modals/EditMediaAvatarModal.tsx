...
export default function EditMediaAvatarModal(props: {
    profileData: ProfileData,
    handleAvatar: (avatarUrl: string) => void,
    handleEditMediaAvatar: (editMediaAvatar: boolean) => void,
}) {
    const { profileData, handleAvatar, handleEditMediaAvatar } = props;
    ...
    async function onClickApply(apply: boolean) {
        try {
            ...
            const croppedImg = await getCroppedImg(profileData.avatar, croppedArea);

            if (!croppedImg) {
                throw Error("Unable to complete getCroppedImg() promise.");
            }

            handleAvatar(croppedImg);
            handleEditMediaAvatar(false);
        } catch (e) {
